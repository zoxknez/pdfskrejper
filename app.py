...
from config.sources import SourceType, get_sources_by_type, get_all_sources
...
def profile():
    ...
    category_stats = db.session.query(
        ScrapingJob.category,
        db.func.count(ScrapingJob.id).label('count')
    ).filter_by(user_id=current_user.id).group_by(ScrapingJob.category).all()
    
    # Total veličina fajlova
    total_size_result = db.session.query(
        db.func.sum(DownloadedFile.file_size)
...
def api_stats():
    ...
    categories_stats = db.session.query(
        DownloadedFile.category,
        db.func.count(DownloadedFile.id)
    ).filter_by(user_id=current_user.id).group_by(DownloadedFile.category).all()
