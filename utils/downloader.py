...
class PDFDownloader:
    ...
    def __init__(
        self, output_dir: Path = None, max_concurrent: int = None, timeout: int = None
    ):
        """
    ...
    async def download_file(
        self, url: str, filename: Optional[str] = None, category: str = "other"
    ) -> Optional[Path]:
        """
        Preuzima jedan fajl.
        ...
        async with self.semaphore:
            try:
                ...
                self.failed_downloads.append(
                    {"url": url, "filename": filename or "unknown", "reason": "Timeout"}
                )
                return None
                ...
                self.failed_downloads.append(
                    {"url": url, "filename": filename or "unknown", "reason": str(e)}
                )
                return None

    async def download_multiple(
        self, urls: List[str], category: str = "other"
    ) -> List[Path]:
        """
        ...
        """
        tasks = [self.download_file(url, category=category) for url in urls]
    ...
    def get_statistics(self) -> Dict:
        ...
        return {
            ...
            "success_rate": (
                ...
                / (len(self.downloaded_files) + len(self.failed_downloads))
                * 100
